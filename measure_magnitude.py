...
def main(args):
    ...
    output_f = args.output_dir / f"mag_exp_{args.f.name}_{args.e}_{args.bs}_{args.s}_{args.r}_{args.dp}.results"

    mags = []
    def after_epoch(e, lstm):
        if e == args.e - 1:
            # add the magnitudes of the words with negative indices
            recovered_tokens, diff = attack.extract_negative_tokens(lstm, gboard_lstm)
            for t in recovered_tokens[0,:]:
                mags.append((tf.math.abs(diff[t]).numpy(),))
    ...
    utils.output_results(output_f, mags)
...
if __name__ == "__main__":
    ...
    output_dirname = f"mag_exp_{str(dt.datetime.now()).replace(' ', '-')[:-7]}"
